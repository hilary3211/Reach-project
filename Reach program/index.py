...
-----+_(___)
    ...
    def play_alice():
        ...
        rpc_callbacks(
            "/backend/Alice",
            ctc_alice,
            dict(wager=rpc("/stdlib/parseCurrency", num), deadline=10, **player(player_1)),
        )
    ...
    def play_bob():
        ans = input("%s would u like to accept the wager\nReply with y/n: "%(player_2))
        if ans == "y":
            ...
            rpc_callbacks(
                "/backend/Bob", ctc_bob, dict(acceptWager=acceptWager, **player(player_2))
            )
